...
namespace UHFPS.Editors
{
    public class ObjectReferencesWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            GUIContent infoText = new("To add elements to the list, Drag and Drop a Folder or Prefabs to the window.");
            infoText = EditorGUIUtility.TrTextContentWithIcon(infoText.text, MessageType.Info);

            GUIStyle infoTextStyle = new(EditorStyles.helpBox);
        ...
        private void AddObjectToTarget(string path)
        {
            if (AssetDatabase.IsValidFolder(path))
            {
                ...
                foreach (string pathGuid in assets)
                {
                    ...
                    {
                        Target.References.Add(new ObjectReferences.ObjectGuidPair()
                        {
                            Object = obj,
                            GUID = pathGuid
                        });
