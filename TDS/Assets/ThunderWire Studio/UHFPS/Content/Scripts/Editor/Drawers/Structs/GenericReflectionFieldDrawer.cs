...
{
    [CustomPropertyDrawer(typeof(GenericReflectionField))]
    public class GenericReflectionFieldDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            using (new EditorGUI.DisabledGroupScope(instanceRef == null))
            {
                ...
                (string display, string name)[] fields = GetReflectionFields(instanceRef, reflectionType, reflectDerived.boolValue);
                int selected = 0;
                ...
                if (fields.Length > 0)
                {
                    ...
                    {
                        if (fields[i].name == reflectName.stringValue)
                        {
                            selected = i;
                            break;
                        }
                ...
                selected = EditorGUI.Popup(reflectNameRect, "Reflect Name", selected, fields.Select(x => x.display).ToArray());

                reflectName.stringValue = fields[selected].name;
        ...
        private (string, string)[] GetReflectionFields(MonoBehaviour instance, ReflectionField.ReflectionType reflectionType, bool reflectDerived)
        {
            ...
            if (instance != null)
            {
                ...
                else
                {
                    fields.AddRange(from method in instanceType.GetMethods(BindingFlags.Public | BindingFlags.Instance)
                                    ...
                                    select ($"{method.ReturnType.Name} {method.Name}", method.Name));
                }
            }

            if (fields.Count == 0)
                fields.Add(("None", string.Empty));
